...
The next major product layers are stronger background processing, richer proactive perspective notes, and those broader MCP write tools after the mutation safety model is stable. The notification foundation is now in place: SSE covers live in-app updates, Web Push covers return-to-app notifications, and the ECS worker can be enabled when VAPID keys are configured. Due reminders are surfaced by time, while ordinary saved memories are ranked by recent context, confidence, source strength, saved intent, recall score, overdue age, and memory type so Crowscap brings back useful ideas without overwhelming the user.
